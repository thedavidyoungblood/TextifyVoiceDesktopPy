...
def verificar_modelo_inicial():
    ...
    if model_path:
        try:
            ...
            # Tentar carregar outros modelos da pasta .model
            diretorio_modelo = resource_path(".model")
            if os.path.exists(diretorio_modelo):
                for filename in os.listdir(diretorio_modelo):
                    if filename.endswith(".pt"):
                        ...
                        try:
                            whisper.load_model(caminho_modelo)
                            logging.info(f"Modelo {caminho_modelo} carregado com sucesso.")
                            config['model_path'] = caminho_modelo
                            with open(CONFIG_FILE, 'w') as f:
                                json.dump(config, f)
                            return  # Modelo válido encontrado, sair da função
                        except Exception as e:
                            logging.error(f"Erro ao carregar o modelo {caminho_modelo}: {e}")
            # Se nenhum modelo válido for encontrado, abrir seleção de qualidade
            selecionar_qualidade()
    else:
        # Tentar carregar modelos da pasta .model
        diretorio_modelo = resource_path(".model")
        if os.path.exists(diretorio_modelo):
            for filename in os.listdir(diretorio_modelo):
                if filename.endswith(".pt"):
    ...
    def baixar_modelo():
        ...
        if os.path.exists(caminho_modelo):
            try:
                ...
                logging.error(f"Erro ao carregar o modelo existente {caminho_modelo}: {e}")
                resposta = messagebox.askyesno("Erro", f"Erro ao carregar o modelo existente: {e}\nDeseja baixar o modelo novamente?")
                if not resposta:
                    janela_qualidade.destroy()
                    return
                else:
